    ...
    return (
        <div className="rounded-3xl bg-white border border-neutral-200/60 shadow-sm p-6 sm:p-7">
            <div className="flex items-center gap-3 mb-6">
                <span className={`w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-md bg-linear-to-br ${accent}`}>
                    {icon}
                </span>
                <div>
        ...
        <div className="max-w-3xl mx-auto space-y-6 pb-40">
            ...
            <Section icon={<FiCalendar className="text-lg" />} title="Calendar" subtitle="How your calendar is displayed" accent="from-sky-400 to-blue-500">
                <Row label="Week starts on" hint="Choose the first day of your week">
                    <SegmentedControl
                        options={weekStarts.map((k) => ({ key: k, label: k }))}
                        value={settings.weekStarts as (typeof weekStarts)[number]}
                ...
                <Row label="Layout density" hint="How much content fits on screen">
                    <SegmentedControl
                        options={densities.map((k) => ({ key: k, label: k }))}
                        value={settings.density as (typeof densities)[number]}
            ...
            <Section icon={<FiUser className="text-lg" />} title="Account" subtitle="Manage your profile and session" accent="from-emerald-400 to-teal-500">
                <Link
                    href="/profile"
                    className="flex items-center justify-between gap-4 py-4 hover:bg-neutral-50/60 rounded-lg -mx-2 px-2 transition-colors"
                >
                    <div className="flex items-center gap-3 min-w-0">
                        <span className="w-11 h-11 rounded-full bg-linear-to-br from-orange-400 to-orange-500 text-white text-sm font-bold flex items-center justify-center shrink-0 shadow-sm">
                            {initial}
                        </span>
                        <div className="min-w-0">
            ...
            <div className="rounded-3xl bg-white border border-neutral-200/60 shadow-sm p-6 sm:p-7">
                <div className="flex items-center gap-3">
                    <span className="w-11 h-11 rounded-xl flex items-center justify-center bg-red-50 text-red-500 shrink-0">
                ...
                <button
                    ...
                    className="w-full inline-flex items-center justify-center gap-2 bg-red-500 text-white px-5 py-3 rounded-xl text-sm font-medium hover:bg-red-600 shadow-md shadow-red-500/15 transition-all duration-200 hover:shadow-lg active:scale-[0.98] mt-6"
                >
            ...
            <Dialog
                ...
                variant="danger"
                title="Log out?"
                description="Are you sure you want to end your current session? You'll be returned to the login screen."
                confirmText="Log out"
                cancelText="Cancel"
                loading={signingOut}
